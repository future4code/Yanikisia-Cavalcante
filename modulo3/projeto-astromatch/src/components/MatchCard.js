import styled from 'styled-components';
import React, { useState, useEffect } from 'react';
import PulseLoader from "react-spinners/PulseLoader";

const AppContainer = styled.div`
    display: flex;
    flex-direction: column;
    min-width: 300px;
    max-height: 600px;
    
    header{
    display: flex;
    height: 10vh;
    justify-content: center;
    border: none;
    border-bottom: 1px solid lightgray;

    }

`
const DivProfiles = styled.div` 
    width: 100%;
    height: 66vh;
    display: flex;
    flex-direction: column;
    align-items: center;
    position: relative;
    justify-content: space-around;
    
    img{
    position: absolute;
    width: 80%;
    height: 90%;
    top: 5%;
    box-shadow: 2px 4px 7px 4px #888888;
    border-radius:10px;
    z-index: 1;
     }
`
const DivInformaçoes = styled.div`
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    color: white;
    z-index: 1;
    width: 80%;
    justify-content: flex-end;
    background-image: linear-gradient(to top, rgba(0,0,0,0.5), transparent);
    border-radius:10px;
    height: 90%;
    
    h3{
        margin-left: 7px;
    }
    
    p{
        padding: 10px;
    }
`

const BotoesEstilizados = styled.div`

    height: 25vh;
    display: grid; 
    width: 100%;
    height: 10vh;
    grid-template-columns: 1fr 1fr;
    grid-template-rows: 1fr;
    justify-items: center;
   
`
const LikeButton = styled.button` 
    border-radius: 50%;
    width: 80px;
    height: 80px;
    border: 1px solid #FD4659;
    color: green;
    font-size: 0;
    transform: scale(0.7);
    transition: all 0.2s ease 0s;
    position: relative;
    box-shadow: rgb(205 205 205 / 73%) 0px 0px 15px 0px;
    overflow: hidden;
    
    img{ 

    width: 40px;
    border-radius: 50%;
    }

    &:hover{
        width: 90px;
        height: 90px;
        background-color: #FD4659;
    }

`
const DeslikeButton = styled.button` 
    border-radius: 50%;
    width: 80px;
    height: 80px;
    border: 1px solid red;
    color: red;
    font-size: 50px;
    transform: scale(0.7);
    transition: all 0.2s ease 0s;
    position: relative;
    box-shadow: rgb(205 205 205 / 73%) 0px 0px 15px 0px;
    overflow: hidden;
 
    &:hover 
    {
    color:white;
    background-color: red;
    width: 90px;
    height: 90px;
    }
`
const Image = styled.img` 
    align-self: center;
    width: 15%;
`
const Title = styled.img`
    width:200px;
`

const CardImage = styled.div`
    position: absolute;
    background-image: url(${props => props.imagemBack});
    width: 90%;
    height: 100%;
    background-size: cover;
    border-radius: 10px;
    filter: blur(10px);
`
export default function MatchCard(props) {
    const [loading, setLoading] = useState(true)

    const callChosseMatch = (choice) => {
        console.log(props)
        const body = {
            id: props.id,
            choice: choice

        }
        props.choose(body, props.getPro)
    }

    const callMathces = () => {
        props.callingScreen("telaMatches")
    }

    return (
        <AppContainer>
            <header>
                <Title src="data:image/png;base64," alt="titulo astro match" />
                <Image onClick={() => callMathces()} src="https://media.istockphoto.com/vectors/user-group-icon-vector-line-art-outline-persons-team-silhouette-with-vector-id1069729916?k=20&m=1069729916&s=170667a&w=0&h=HG8U5zkoKNjkk_sXMPpmR6IJi0e-JffPhFxfr9YAvTM=" alt="botao para ir para matches" />
            </header>
            <DivProfiles>
                {props.possibles.name ?
                    <>
                        <CardImage imagemBack={props.possibles.photo} >
                        </CardImage>
                        <img src={props.possibles.photo} />
                        <DivInformaçoes>
                            <h3>{`${props.possibles.name}, ${props.possibles.age}`}</h3>
                            <p>{props.possibles.bio}</p>
                        </DivInformaçoes>
                    </>
                    : <PulseLoader
                        loading={loading}
                        color="#FFAB0F"
                        size={50}
                        speedMultiplier={2}
                    />}
            </DivProfiles>
            <BotoesEstilizados>
                <DeslikeButton onClick={() => callChosseMatch(false)}>X</DeslikeButton>
                <LikeButton onClick={() => callChosseMatch(true)}><img src="https://i1.wp.com/www.multarte.com.br/wp-content/uploads/2018/10/coracao-png2.png?resize=220%2C220&ssl=1" alt="botao para ir para matches" /></LikeButton>
            </BotoesEstilizados>
        </AppContainer>
    );
}

