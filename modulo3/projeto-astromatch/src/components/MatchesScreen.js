...
function MatchesScreen(props) {
  ...
  const mapList = props.match.map((item) => {
    return <DivMatches key={Math.random()}>
      <Imagem imagem={item.photo}></Imagem>
      <p>{item.name}</p>
    </DivMatches>
  })
  ...
  return (
    <AppContainer>
      <header>
        <Image onClick={() => props.callingScreen("profiles")} src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQwq-4b5GROh25UPTOehUZbxSvKiiMiz3CWD6tRiTEPwl4WGXAiBz4b8Tgjn4QOMLgPkhs&usqp=CAU" alt="botao para ir para matches" />
        ...
        <ImageEx onClick={Cleaning} src="https://cdn.pixabay.com/photo/2012/04/10/23/56/cross-27169_960_720.png" alt="botao para ir para matches" />
      </header>
      <Ai>
        {props.match.length == 0 ? <p>Ainda sem matches</p> : mapList}
      </Ai>
    </AppContainer>
  );
}
